...
@section('content')
    <div class="container">
        <h1 class="mb-4 text-center">{{ __('إضافة مجموعة جديدة') }}</h1>
                ...
                <form action="{{ route('groups.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('kutab_id') is-invalid @enderror" id="kutab_id" name="kutab_id" required>
                            ...
                            @foreach ($kutabs as $kutab)
                                <option value="{{ $kutab->id }}" {{ isset($defaultKutab) && $defaultKutab->id == $kutab->id ? 'selected' : '' }}>
                                    {{ $kutab->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-select @error('week_day_id') is-invalid @enderror" id="week_day_id" name="week_day_id">
                            ...
                            @isset($weekDays)
                                @foreach ($weekDays as $weekDay)
                                    <option value="{{ $weekDay->id }}" {{ old('week_day_id') == $weekDay->id ? 'selected' : '' }}>
                                        {{ $weekDay->name }}
                                    </option>
                                @endforeach
                            @endisset
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control @error('hour') is-invalid @enderror" id="hour" name="hour" value="{{ old('hour') }}" min="0" max="23">
                            @error('hour')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control @error('minute') is-invalid @enderror" id="minute" name="minute" value="{{ old('minute', 0) }}" min="0" max="59">
                            @error('minute')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-success mobile-button">{{ __('حفظ') }}</button>
                        <a href="{{ route('groups.index', ['kutab_id' => request('kutab_id')]) }}" class="btn btn-secondary mobile-button">{{ __('إلغاء') }}</a>
                    </div>
                </form>
            </div>
        </div>
